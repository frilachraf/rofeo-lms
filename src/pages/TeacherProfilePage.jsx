...
export default function TeacherProfilePage() {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.full_name) newErrors.full_name = "Nom requis.";
    if (!formData.email) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (file) {
      const { data: uploadData, error: uploadError } = await uploadTeacherAvatar(user.id, file);
      if (uploadError) {
    ...
    const { error } = await editTeacherAccount(user.id, updates);
    if (error) {
    ...
    <div className="max-w-4xl mx-auto py-10 space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Supprimer votre compte définitivement</CardDescription>
        </CardHeader>
        <CardContent>
