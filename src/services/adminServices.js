...
export const getStudents = async () => {
  const {data, error} = await supabase
    ...
    if(error){ return handleError(error, 'Erreur lors de la récupération des étudiants') }
  return {data, error}
}
...
export const getStudent = async (studentId) => {
  const {data, error} = await supabase
    ...
    .eq('id', studentId)
    .single()
    if(error){ return handleError(error, 'Erreur lors de la récupération de l\'étudiant') }
  return {data, error}
}
...
export const getCourses = async () => {
  const {data, error} = await supabase
    ...
    if(error){ return handleError(error, 'Erreur lors de la récupération des cours') }
  return {data, error}
}
...
export const getCourse = async (courseId) => {
  const {data, error} = await supabase
    ...
    .eq('id', courseId)
    .single()
    if(error){ return handleError(error, 'Erreur lors de la récupération du cours') }
  return {data, error}
}
...
export const getCoursesCount = async () => {
    const { count, error } = await supabase.from('courses')
        .select("*", { count: 'exact' })
        if(error){ return handleError(error, 'Erreur lors de la récupération du nombre de cours') }
    return { count, error }
}
export const getStudentsCount = async () => {
    const { count, error } = await supabase.from('students_accounts')
        .select("*", { count: 'exact' })
        if(error){ return handleError(error, 'Erreur lors de la récupération du nombre d\'étudiants') }
    return { count, error }
}
export const getTodayEnrollmentsCount = async () => {
    ...
    const { count, error } = await supabase
        ...
        if(error){ return handleError(error, 'Erreur lors de la récupération du nombre d\'inscriptions aujourd\'hui') }
    return { count, error }
}
